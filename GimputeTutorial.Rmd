...
```{r eval=FALSE} 
## Start a new R session after you define a directory where your data processing and imputation results will be stored.
...
perl4snpsWithSamePos = paste0(GimputeDIR,"src//snpsWithSamePos.pl")
alignScript4compareName = paste0(GimputeDIR,"src/dataSetImpRefCompareName.pl ")
alignScript = paste0(GimputeDIR,"src/dataSetImpRefComparePos.pl ")
```
...
Input files:
* Raw chip data of the instances in PLINK format: ../0-rawData/plinkFiles/*.#
* Files with meta information about the instances: ../0-rawData/sampleInfo/*.txt
* Chip annotation file for converting the chip data to a targeted genome build (see section 2.4.2).
...
* Genotype data of instances for a specific genome build:../1-conversion/1_11_removedYMtSnp.#
...
```
14. For 2_13_removedSnpHweAlloCt.# calculate the PCA (with plots) and remove the outliers from that dataset (for the definition of the outliers see section 1.1.3).
...
## Alignment with the imputation reference

All files named in this subsection are in the directory 3-lifting/.  

Input files:

* QC instance data within HWE tests for a specific genome build (see section 2.4.2): 2_14_removedSnpHweAlloAll.#
...
Do that for the QC instance data within HWE tests.
...
* Lifted instance data for the genome build of the imputation reference files: ../3-lifting/3_4_removedSnpDiffAlleles.#
* Imputation reference files (see section 1.1.4).
...
# Extending pipelines
Ongoing embeded tools

# FAQ: Trouble-shooting 
Contact: junfang.chen3@gmail.com